...
#define DRIVE_STTAIGHT_HEADING 0;
...
bool* Drive_Init(float Kp, float Ki, float Kd, int left, int right, float diameter){
	...
	DriveTaskVals.target = 0;
	DriveTaskVals.max_velocity = 0;
	DriveTaskVals.finishedFlag = false;

	wheel_pid.finishedFlag = false;
...
task Drive_Straight(){
	...
	do{
		hogCPU();
		int heading_correction = (DRIVE_STRAIGHT_HEADING - getGyroDegreesFloat(gyro_vals.gyro_port)) * gyro_vals.headingKp;
	...
	do{
		...
		if(!reversed){
			if(sgn(gyro_vals.gyro_pid.target) > 0){
				motor[wheel_pid.right_motor] = gyro_val.gyro_pid.output;
				motor[wheel_pid.left_motor] = 0;
			}else{
				motor[wheel_pic.left_motor] = -gyro_vals.gyro_pid.output;
				motor[wheel_pid.right_motor] = 0;
			}
		}else{
			if(sgn(gyro_vals.gyro_pid.target) > 0){
				motor[wheel_pid.left_motor] = -gyro_val.gyro_pid.output;
				motor[wheel_pid.right_motor] = 0;
			}else{
				motor[wheel_pic.right_motor] = gyro_vals.gyro_pid.output;
				motor[wheel_pid.left_motor] = 0;
			}
		}
		releaseCPU();
